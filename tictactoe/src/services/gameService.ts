...
export async function makeMove(
  roomId: string,
  index: number,       // posição no tabuleiro (0 a 8)
  player: Player,
  currentBoard: Cell[]
  ...
  if (currentBoard[index] !== null) {
    return { success: false, error: "Célula ocupada!" };
  }

  // Cria o estado do tabuleiro
  const newBoard = [...currentBoard];    // copia o array atual
  newBoard[index] = player;             // marca a célula
...
export function subscribeToRoom(
  ...
  onValue(roomRef, (snapshot) => {
    ...
    // Garante que board seja sempre uma string válida de 9 chars
